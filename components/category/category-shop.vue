<template>
  <section class="tp-category-area pb-120">
    <div class="container">
      <div class="row">
        <div v-for="item in category_data" :key="item.id" class="col-lg-4 col-sm-6">
          <div
          ...
          >
            <div class="tp-category-main-content">
              <h3 class="tp-category-main-title pb-1">
                <a @click="handleCategory(item.parentName)" class="pointer">{{item.parentName}}</a>
              </h3>
...
const handleCategory = (parent:string) => {
  router.push(
    `/shop?category=${parent.split(" ").join("-").toLowerCase()}`
  );
};
</script>
